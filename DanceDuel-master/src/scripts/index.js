...
function adjustNumberInput(targetElement, minThreshold, maxThreshold) {
  ...
  if (!targetElement || typeof minThreshold !== 'number' || typeof maxThreshold !== 'number') {
    console.error(
      'Недопустимые аргументы. Пожалуйста, укажите допустимый целевой элемент, минимальный порог и максимальный порог.'
    );
    return;
  }
...
function adjustInputRegex(targetElement, regex) {
  ...
  if (!targetElement || !(regex instanceof RegExp)) {
    console.error('Недопустимые аргументы. Пожалуйста, укажите допустимый целевой элемент и регулярное выражение.');
    return;
  }
...
function adjustInputLength(targetElement, maxLength) {
  ...
  if (!targetElement || typeof maxLength !== 'number' || maxLength <= 0) {
    console.error('Недопустимые аргументы. Пожалуйста, укажите допустимый целевой элемент и положительную максимальную длину.');
    return;
  }
...
function arena(firstHero, secondHero) {
  console.log(`Да пусть начнется танцевальная битва между ${firstHero.name} и ${secondHero.name}!`);

  const fistHeroSum = countStatsSum(firstHero);
  const secondHeroSum = countStatsSum(secondHero);

  console.log('Сумма значений для первого героя: ', fistHeroSum);
  console.log('Сумма значений для второго героя: ', secondHeroSum);

  const winner = fistHeroSum > secondHeroSum ? firstHero : fistHeroSum < secondHeroSum ? secondHero : null;

  if (winner) {
    console.log(`Ритмично аплодируйте победителю:\n${winner.name}`);
    alert(`Ритмично аплодируйте победителю: ${winner.name}`);
  } else {
    console.log('В танцевальной битве торжествует дружба!');
    alert('В танцевальной битве торжествует дружба!');
  }
}
...
sendToBattleButton.onclick = () => {
  ...
  if (heroName !== '') {
    ...
    alert('Добавьте имя своему герою!');
  }
};
...
doSkillButton.onclick = () => {
  if (playerHero) {
    if (playerHero.constructor.name === 'Маг') {
      playerHero.healHero(playerHero);
    } else if (playerHero.constructor.name === 'Рыцарь') {
